...
def train_one_split(
    ...
    train_dataloader = get_SWD_dataloader(piece_list=train_list, seg_hop_length=train_seg_hop_length, shuffle=True, **config["data"])
    val_dataloader = get_SWD_dataloader(piece_list=val_list, seg_hop_length=val_seg_hop_length, shuffle=False, **config["data"])
    ...
    print("{:.2f}K parameters!".format(sum([np.prod(x.shape) for x in model.parameters()]) / 1e3))
    ...
    for epoch in range(max_epoch):
        print("{} | Epoch: {} Training...".format(datetime.now().strftime("[%m/%d - %H:%M:%S]"), epoch))
        model.train()
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            writer.add_scalars("lr", dict(lr=optimizer.param_groups[0]["lr"]), num_steps)
        
        print("{} | Epoch: {} Validating...".format(datetime.now().strftime("[%m/%d - %H:%M:%S]"), epoch))
        model.eval()
        val_loss = []
        correct, count = 0, 0
        with torch.no_grad():
        ...
        print("{} | Epoch: {} Validation Loss={:.2f} Accuracy={:.2f}".format(
            datetime.now().strftime("[%m/%d - %H:%M:%S]"), epoch, val_loss, acc))
        ...
        if acc > best_acc:
            ...
            if ckpt_path is not None:
                os.remove(ckpt_path)
            ckpt_path = os.path.join(save_dir, "epoch={:d}_loss={:.3f}_acc={:.2f}.pth".format(epoch, val_loss, acc))
...
def train(config):
    ...
    for i in range(num_split):
        print("{} | Split: {}".format(datetime.now().strftime("[%m/%d - %H:%M:%S]"), i))
        train_one_split(
            train_list=train_list[i],
            val_list=val_list[i],
            save_dir=os.path.join("{}/split_{:d}".format(base_save_dir, i)),
            config=config
        )
